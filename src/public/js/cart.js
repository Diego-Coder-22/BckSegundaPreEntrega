...
async function deleteProductFromCart(cid, pid) {
    ...
    try {
        const response = await fetch(`https://bcksegundapreentrega-production.up.railway.app/api/carts/${cid}/products/${pid}`, {
            method: 'DELETE',
            headers: {
...
async function clearCart(cid) {
    try {
        const response = await fetch(`https://bcksegundapreentrega-production.up.railway.app/api/carts/${cid}`, {
            method: 'DELETE',
            headers: {
                "authorization": `Bearer ${token}`,
            },
